...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cm_") ; then
       DREAM_BUILD=$(echo -n $1 | sed -e 's/^cm_//g')
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${DREAM_DEVICES_ONLY}" != "z" ]; then
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var DREAM_VERSION)
        ZIPFILE=DreamOS-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.dream.device=$DREAM_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $DREAM_BUILD, run away!"
    fi
}
...
function installboot()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.cm.device=$DREAM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.cm.device=$DREAM_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $DREAM_FIXUP_COMMON_OUT ]; then
